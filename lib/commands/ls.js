...
var inspectRoute = require('./inspectRoute.js');
...
var Command = {
     ...
     */
    build: function(cli) {
        return cli.vantage.command('ls [filter...]')
        .option('-a, --apps', 'Lists apps')
        .option('-r, --routes [uid]', "Lists app's routes")
        .option('-s, --sort <method>', "Defines sort order. Can contain characters m|u|i|d which corresponds to (m)ethod, (u)rl, u(i)d and mi(d)dleware columns")
    ...
    action: function action(cli) {
        ...
        return function(args, callback) {
            ...
            if (args.options.routes) {
                ...
                if (typeof args.options.routes === 'boolean') {
                ...
                } else { //inspect specific route
                    return inspectRoute.action(cli).call(this, {
                        uid: args.options.routes,
                        options: {}
                    }, callback);
                }
            }
    ...
    printApps: function printApps(apps, opt) {
        ...
        apps.forEach(function(app, index) {
            var port = 'undefined';
            if (app.server && app.server.address instanceof Function ) {
    ...
    printRoutes: function printRoutes(apps, options) {
        ...
        _.compact(apps).forEach(function(app, index) {
            ...
            routers.forEach(function(router) {
                ...
                routes.forEach(function(route) {
                    ...
                    var sdkMethodName = route.description && route.description.sdkMethodName;
                    ...
                    data.url = route.Router.$normalizeUrl(data.url);
        ...
        return Table.print(routeList, function(data, cell) {
            ...
            cell('url', data.url);
            cell('uid', data.uid);
            cell('sdkMethod', data.sdkMethodName);
